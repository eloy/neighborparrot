...
Proxy between RabbitMQ and EventSource 
via Goliath
...
=== Receive messages
    curl -d "channel=prueba""localhost:9000/open"

=== Send messages
    curl POST -d "channel=prueba" -d "payload=Hello world!" localhost:9000/open

== Deploy in ubuntu

<pre><code>
sudo aptitude install ruby1.9.1 ruby1.9.1-dev git-core erlang-nox
...
cd /etc/puppet/modules && puppet-module install puppetlabs/rabbitmq && cd 
